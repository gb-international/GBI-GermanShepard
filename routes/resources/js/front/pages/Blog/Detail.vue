    ...
    <div class="container mt-2">
      <div class="blog-detail">
        <p class="text-muted pt-3 pb-3" v-if="posts.category">
          <router-link :to="`/blog`">GBI Travel Blog </router-link>>
          <router-link
            :to="`/blog/category/${posts.category.slug}`"
...
export default {
  ...
  created() {
    ...
    var metaInfo = {
      ...
      url: `https://www.gowithgbi.com/blog/${this.posts.slug}`,
      type: 'website'
    }
    document.cookie = "GBIMeta =" + JSON.stringify(metaInfo) +"; path=/";
  },
  methods: {
    getBlogDetail() {
      this.$axios
        .get(`/api/getpost/${this.$route.params.slug}`)
        .then((response) => {
    ...
    getRelatedBlogs() {
      this.$axios
        .get(`/api/related-blog/${this.posts.category_id}`)
        .then((response) => {
