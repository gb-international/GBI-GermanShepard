  ...
  methods: {
    ...
    sendLoginCreds(){
      ...
      for (var i = 0; i < this.selected.length; i++) {
            axios
            .post("/api/corp-group/send-member-login", this.selected[i], {
            })
            .then(response => {
     ...
     checkAndUpdateRow(data, index){
      ...
      axios
          .post("/api/corp-group/update", data, {
            })
            .then(response => {
             this.$swal.fire(
                "Success",
    ...
    checkAndSave(){
      for (var i = 0; i < this.new_row.length; i++) {
        ...
        //this.UserGroupSave();
        axios
            .post("/api/corp-group/add", this.new_row, {
            })
            .then(response => {
             this.$swal.fire(
                "Success",
    ...
    groupMember() {
      axios
        .get(`/api/corp-group/${this.$route.params.id}/corporate`)
        .then((res) => {
            if (res.data) {
