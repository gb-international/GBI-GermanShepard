...
export default {
  ...
  methods: {
    ...
    submitPayment() {
      ...
      if(this.teacherform.payment_mode == "self" &&
        this.teacherform.payment_type == "net"){
        this.NetPayment();
      }
    ...
    NetPayment() {
      ...
      axios.post("/api/admin-payment", this.teacherform)
        .then((res) => {
          if (res.data["error"]) {
            this.$swal.fire({
              icon: "error",
      ...
      data.user_id = this.teacherform.user_id;
      //data.paid_person = this.teacherform.paid_person;
