<!--
This Template is for listing for the Jobs using function to get the 
data from the api to display the data about the Jobs from the backend .
-->
<template>
  <list-layout addurl="/jobs-add" buttontext="add Job">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <edit-icon :url="`/jobs/${data.item.id}`"></edit-icon>
          <delete-icon 
...
export default {
  ...
  data() {
    return {
      fields: [
        {key:'title',label:'Job Title',sortable:true,thClass: 'table-head'},
        {key:'job_type',label:'Job Type',sortable:true,thClass: 'table-head'},
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/jobs/"+id,index,'index':index};
