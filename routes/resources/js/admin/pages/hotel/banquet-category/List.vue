...
<template>
  <list-layout addurl="/banquet-categories-add" buttontext="add">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <edit-icon :url="`/banquet-categories/${data.item.id}`"></edit-icon>
          <delete-icon 
            @click.native="deleteItem(data.item.id,data.index)"
            >
          </delete-icon>
          <view-icon :url="`/banquet-categories-view/${data.item.id}`"></view-icon>
        </template>
      </b-table> 
    </template>
...
export default {
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/banquet-categories/"+id,index,'index':index};
