...
<template>
  <list-layout addurl="/room-categories-add" buttontext="add">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <edit-icon :url="`/room-categories/${data.item.id}`"></edit-icon>
          <delete-icon 
            @click.native="deleteItem(data.item.id,data.index)"
            >
          </delete-icon>
          <view-icon :url="`/room-categories-view/${data.item.id}`"></view-icon>
        </template>
      </b-table> 
    </template>
...
export default {
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/room-categories/"+id,index,'index':index};
