                ...
                <div class="top_btn">
                  ...
                  <table id="example" class="display table table-striped table-bordered nowrap" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr v-for="tourprogram in tourprogram_list" role="row" v-bind:class="{ odd: oddclass , 'even': evenclass}" class="row_list">
                            <td>{{ tourprogram.title }}</td>
                            <td><img :src="getImgUrl(tourprogram.image)"></td>
                            <td><span v-html="tourprogram.description"></span></td>
                            <td class="edit_section">
                              <router-link :to="`tourprogram/${tourprogram.id}`" class="edit_link">
...
export default {
  ...
  created(){
   ...
   methods:{
    encyclopediaData(){
      axios.get('/api/tourprogram').then((response)=>{
        setTimeout(() => $('#example').DataTable(), 1000);
        this.tourprogram_list = response.data;
