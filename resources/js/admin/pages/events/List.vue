    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <edit-icon :url="`/edit-event/${data.item.id}`"></edit-icon>
          <delete-icon 
...
export default {
  ...
  data() {
    return {
      fields: [
        {key:'name',label:'Event',sortable:true,thClass: 'table-head'},
        {key:'date',label:'Date',sortable:true,thClass: 'table-head'},
        {key:'time',label:'Time',sortable:true,thClass: 'table-head'},
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/events/"+id,index,'index':index};
