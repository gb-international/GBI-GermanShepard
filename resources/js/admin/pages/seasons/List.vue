<!--
This Template is for listing for the Seasons using function to get the 
data from the api to display the data about the Seasons from the backend .
-->
<template>
  <list-layout addurl="/add-season" buttontext="add season">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <edit-icon :url="`/edit-season/${data.item.id}`"></edit-icon>
          <delete-icon 
...
export default {
  ...
  data() {
    return {
      fields: [
        {key:'name',label:'Season',sortable:true,thClass: 'table-head'},
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/season/"+id,index,'index':index};
