...
-->
<template>
  <list-layout addurl="/add-popular-itinerary" buttontext="add Popular Itinerary">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <edit-icon :url="`/edit-popular-itinerary/${data.item.id}`"></edit-icon>
          <delete-icon 
...
export default {
  ...
  data() {
    return {
      fields: [
        {key:'itinerary.title',label:'Name',sortable:true,thClass: 'table-head'},
        {key:'itinerary.source',label:'Source',sortable:true,thClass: 'table-head'},
        {key:'itinerary.destination',label:'Destination',sortable:true,thClass: 'table-head'},
        {key:'season.name',label:'Season',sortable:true,thClass: 'table-head'},
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/popular-itineraries/"+id,index,'index':index};
