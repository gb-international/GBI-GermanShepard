    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <view-icon :url="`/userpayments-view/${data.item.id}`"></view-icon>
          <delete-icon 
...
export default {
  ...
  data() {
    return {
      fields: [
        {key:'tour_code',label:'TOUR CODE',sortable:true,thClass: 'table-head'},
        {key:'amount',label:'AMOUNT',sortable:true,thClass: 'table-head'},
        {key:'added_by',label:'ADDED BY',sortable:true,thClass: 'table-head'},
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/userpayments/"+id,index,'index':index};
