...
-->
<template>
  <list-layout addurl="/flight-add" buttontext="add flight">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <view-icon :url="`/flight-view/${data.item.id}`"></view-icon>
          <edit-icon :url="`/flight/${data.item.id}`"></edit-icon>
          <delete-icon 
...
export default {
  ...
  data() {
    return {
      fields: [
        {key:'code',label:'air number',sortable:true,thClass: 'table-head'},
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/flight/"+id,index,'index':index};
