...
-->
<template>
  <list-layout addurl="/train-add" buttontext="add train">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <view-icon :url="`/train-view/${data.item.id}`"></view-icon>
          <edit-icon :url="`/train/${data.item.id}`"></edit-icon>
          <delete-icon 
...
export default {
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/train/"+id,index,'index':index};
