...
-->
<template>
  <list-layout addurl="/add-role" buttontext="add role">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          <edit-icon :url="`/edit-role/${data.item.id}`"></edit-icon>
          <delete-icon 
...
export default {
  ...
  data() {
    return {
      fields: [
        {key:'name',label:'ROLE NAME',sortable:true,thClass: 'table-head'},
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/role/"+id,index,'index':index};
