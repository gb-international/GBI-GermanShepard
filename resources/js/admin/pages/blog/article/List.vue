...
-->
<template>
  <list-layout addurl="/articles-add" buttontext="add new">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(action)="data">
          ...
          <edit-icon :url="`/articles/${data.item.id}`"></edit-icon>
          <delete-icon 
            @click.native="deleteItem(data.item.id,data.index)"
            >
          </delete-icon>
          <view-icon :url="`/articles-view/${data.item.id}`"></view-icon>
        </template>
      </b-table> 
    </template>
...
export default {
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/articles/"+id,index,'index':index};
      this.$store.dispatch('deleteItem',payload);
    },
    publishItem(id, user_id) {
      axios.post("/api/articles/"+id+"/publish/"+user_id).then((res) => {});
      /*setTimeout(() =>
    ...
    draftItem(id, user_id){
      axios.post("/api/articles/"+id+"/draft/"+user_id).then((res) => {});
      this.getitems();
      this.$router.go()
    }
  },
