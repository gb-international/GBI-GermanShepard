<!--
Creator: @Manas
This Template is for listing companies.
-->
<template>
  <list-layout addurl="/add-company" buttontext="add company">
    <template #perpage>
      <b-form-group
        label="Per page"
    ...
    <template #table>
      ...
      >
        ...
        <template #cell(incharge_name)="data">
          <div v-if="data.item.incharge != null">
            <router-link :to="`/user/${data.item.incharge.id}`" class="font-weight-bold">
                {{ data.item.incharge.name }}
            </router-link>
          </div>
          <div v-else title="Send Login Details by clicking edit button">{{ data.item.incharge_name }}</div>
        </template>
        <template #cell(action)="data">
          <view-icon :url="`/view-company/${data.item.id}`"></view-icon>
          <edit-icon :url="`/company/${data.item.id}`"></edit-icon>
          <delete-icon 
...
export default {
  ...
  data() {
    return {
      fields: [
        {key:'company_name',label:'name',sortable:true,thClass: 'table-head'},
        {key:'incharge_email_id',label:'company email',sortable:true,thClass: 'table-head'},
        {key:'incharge_name',label:'contact person',sortable:true,thClass: 'table-head'},
  ...
  methods: {
    ...
    deleteItem(id,index=-1) {
      let payload = {'api':"/company/"+id,index,'index':index};
