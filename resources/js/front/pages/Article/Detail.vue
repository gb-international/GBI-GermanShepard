  ...
  <div id="BlogDetail">
    ...
    <div class="container mt-2">
      <div class="blog-detail">
        <p class="text-muted pt-3 pb-3" v-if="posts.category">
          <router-link :to="`/article`">GBI Articles </router-link>>
          <router-link
            :to="`/article/category/${posts.category.slug}`"
        ...
        <div class="text-left mb-2 author-blog">
          ...
          <p class="m-0 font-14 text-muted">
            GBI-Author<br />Last updated : {{ posts.updated_at }}
          </p>
        </div>
        <div class="description mb-2">
          <!-- Write Description here -->
          <p class="largeFirstLetter" v-html="posts.description"></p>
          <div class="mt-3">
            <span
              class="text-dark card-tag mr-2 p-1 pr-2 pl-2"
            ...
            <div class="col-sm-8">
              <form class="form" @submit.prevent="addComment()">
                <div class="form-group">
                  <input
                    type="text"
                    v-model="form.name"
                    class="form-control border-1-grey"
                  ...
                  <p v-if="error_message != ''" class="text-danger">
                    {{ error_message }}
                  </p>
                  <div class="row">
              ...
              >
                <img
                  ...
                  alt="John Doe"
                  class="avatar-author mr-3 mt-3 rounded-circle"
                />
                <div class="media-body">
                  <h6>
                    {{ comment.name }}
                    <small
                      ><i>{{ comment.created_at }}</i></small
                    >
                  </h6>
...
export default {
  name: "ArticleDetail",
  components: { VueSlickCarousel, Form, HasError, ArticleCard },
  ...
  metaInfo() {
    return {
      ...
      meta: [
        ...
        {
          name: "url",
          content: `https://www.gowithgbi.com/article/${this.posts.slug}`,
        },
  ...
  created() {
    ...
    var metaInfo = {
      ...
      url: `https://www.gowithgbi.com/article/${this.posts.slug}`,
      type: 'website'
    }
    document.cookie = "GBIMeta =" + JSON.stringify(metaInfo) +"; path=/";
  },
  methods: {
    getBlogDetail() {
      this.$axios
        .get(`/api/getarticle/${this.$route.params.slug}`)
        .then((response) => {
    ...
    getRelatedBlogs() {
      this.$axios
        .get(`/api/related-article/${this.posts.category_id}`)
        .then((response) => {
